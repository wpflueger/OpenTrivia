...
function validateRound(round: unknown, index: number): void {
  ...
  if (typeof r.id !== "string") {
    throw new PackValidationError(
      `round[${index}].id must be a string`,
      `rounds[${index}].id`,
    );
  }
  ...
  if (!Array.isArray(r.questions) || r.questions.length === 0) {
    throw new PackValidationError(
      `round[${index}].questions must be a non-empty array`,
    );
  }
...
function validateQuestionRef(
  ...
  if (typeof ref !== "object" || ref === null) {
    throw new PackValidationError(
      `round[${roundIndex}].questions[${refIndex}] must be an object`,
    );
  }
...
export function validateQuestion(data: unknown): data is Question {
  ...
  if (!["mcq", "boolean"].includes(q.type as string)) {
    throw new PackValidationError(
      'question.type must be "mcq" or "boolean"',
      "type",
    );
  }
  ...
  if (typeof q.prompt !== "string" || q.prompt.length === 0) {
    throw new PackValidationError(
      "question.prompt must be a non-empty string",
      "prompt",
    );
  }
  ...
  if (!Array.isArray(q.choices) || q.choices.length < 2) {
    throw new PackValidationError(
      "question.choices must have at least 2 choices",
      "choices",
    );
  }
  ...
  if (typeof q.answer !== "object" || q.answer === null) {
    throw new PackValidationError(
      "question.answer must be an object",
      "answer",
    );
  }
  ...
  if (typeof answer.choiceId !== "string") {
    throw new PackValidationError(
      "question.answer.choiceId must be a string",
      "answer.choiceId",
    );
  }
...
function validateChoice(data: unknown, index: number): data is Choice {
  ...
  if (typeof c.text !== "string" || c.text.length === 0) {
    throw new PackValidationError(
      `choice[${index}].text must be a non-empty string`,
    );
  }
