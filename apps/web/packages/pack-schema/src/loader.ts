...
export class PackLoader {
  ...
  private async fetchJson<T>(path: string): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new PackLoadError(`Failed to fetch ${path}: ${response.statusText}`, response.status);
      }
...
export function parseGitUrl(url: string): { owner: string; repo: string; path?: string; ref?: string } {
  const patterns = [
    /github\.com\/([^\/]+)\/([^\/]+?)(?:\.git)?$/,
    /github\.com\/([^\/]+)\/([^\/]+?)\/tree\/([^\/]+)\/(.+)$/,
  ];
...
export function getRawContentUrl(owner: string, repo: string, ref = 'main', path = ''): string {
