...
describe('Game Flow Integration Tests', () => {
  ...
  describe('Full Game Flow', () => {
    ...
    it.skip('should track final scores correctly after full game', () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const player2Answer = questionAnswers[i] === 'b' ? 'a' : (questionAnswers[i] === 'true' ? 'false' : 'b');
        ...
        ctx.players.forEach((player) => {
          const isCorrect = player.answers[0] === ctx.questions[i].answer.choiceId;
          if (isCorrect) {
            player.score += 1000;
          }
        });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle single player game', () => {
      const singlePlayer = new Map([['player-1', { nickname: 'Solo', score: 0, answers: [] }]]);
    ...
    it('should handle empty answers (no submission)', () => {
      ...
      if (answer) {
        const isCorrect = answer === ctx.questions[ctx.hostState.currentQuestion].answer.choiceId;
        if (isCorrect) player.score += 1000;
      }
  ...
  describe('Scoring Edge Cases', () => {
    it('should calculate time bonus correctly at different speeds', () => {
      ...
      scenarios.forEach(({ timeMs, expected }) => {
        const timeBonus = Math.max(0, Math.floor((timeLimit - timeMs) / 1000) * 100);
...
describe('WebRTC Signaling Integration', () => {
  describe('Connection Flow', () => {
    it('should complete signaling exchange', async () => {
      const session: { offer?: RTCSessionDescriptionInit; answer?: RTCSessionDescriptionInit; candidates: RTCIceCandidateInit[] } = {
        candidates: [],
      };
