...
const mockPlayers = [
  ...
  { id: 'p3', nickname: 'Charlie', isReady: true, isConnected: false, score: 0 },
];
...
describe('Game Store', () => {
  ...
  describe('removePlayer', () => {
    it('should remove a player from the game', () => {
      ...
      expect(useGameStore.getState().players.find(p => p.id === 'p1')).toBeUndefined();
    });
  });
  ...
  describe('setQuestions', () => {
    it('should set the questions for the game', () => {
      ...
      expect(useGameStore.getState().questions[0].prompt).toBe('What is the capital of France?');
    });
  });
  ...
  describe('startGame', () => {
    ...
    it('should shuffle questions when shuffleQuestions is true', () => {
      const { addPlayer, setQuestions, startGame, settings } = useGameStore.getState();
      
      useGameStore.setState({ settings: { ...settings, shuffleQuestions: true } });
      mockPlayers.forEach(addPlayer);
      setQuestions(mockQuestions);
      startGame();
  ...
  describe('showQuestion', () => {
    ...
    it('should clear previous answers', () => {
      const { showQuestion, addPlayer, submitAnswer, setQuestions } = useGameStore.getState();
  ...
  describe('submitAnswer', () => {
    ...
    it('should not accept answers when locked', () => {
      const { showQuestion, lockQuestion, submitAnswer } = useGameStore.getState();
    ...
    it('should calculate time bonus correctly', () => {
      ...
      const timeBonus = Math.floor((settings.questionTimeLimit - 10000) / 1000) * 100;
  ...
  describe('nextQuestion', () => {
    it('should advance to next question', () => {
      const { startGame, showQuestion, nextQuestion, setPhase } = useGameStore.getState();
  ...
  describe('reset', () => {
    it('should reset all state to initial values', () => {
      const { addPlayer, setQuestions, setRoomId, startGame, reset } = useGameStore.getState();
